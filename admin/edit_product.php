    ...
    if (empty($product_name) || empty($sku) || empty($price) || empty($category)) {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            $stmt = $pdo->prepare("UPDATE products SET 
                                  ...
                                  price = ?, category = ?, product_image = ?, 
                                  stock_quantity = ?, is_featured = ? 
                                  WHERE id = ?");
            
            if ($stmt->execute([
                $product_name, $sku, $product_description, $price, $category,
                $product_image, $stock_quantity, $is_featured, $product_id
            ])) {
...
<head>
    ...
    <title>Edit Product - Bitronics</title>
    ...
    <style>
        /* Reuse the same styles from admin/add_product.php */
        :root {
            --primary: #0f2f4d;
                ...
                <div class="form-group">
                    ...
                    <div class="image-preview" id="imagePreview">
                        <?php if ($current_image): ?>
                            <img src="../<?php echo htmlspecialchars($current_image); ?>" alt="Current product image">
                        <?php else: ?>
